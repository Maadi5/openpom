...
# ─────────────────────────────────────────────────────────────────────────────
# We'll use EGNN as our graph backbone:
...
class WeightedBCE(nn.Module):
    def __init__(self, pos_weights: torch.Tensor):
        super().__init__()
        self.pos_weights = pos_weights  # (num_labels,)
...
def featurize_smiles_enhanced(smiles: str, radius3d: float):
    """
    Returns a dict with:
      'x':         (num_atoms, 108)  FloatTensor – per‐atom features
      'pos':       (num_atoms, 3)    FloatTensor or None
      'mask':      (num_atoms,)      BoolTensor
      'edge_index':(2, num_edges)    LongTensor
      'edge_attr': (num_edges, 4)    FloatTensor – one-hot bond type
    """
    ...
    try:
        if radius3d > 0:
            ...
            if res == 0 and mol.GetNumConformers() > 0:
                pos_np = mol.GetConformer(0).GetPositions()
                coords = torch.tensor(pos_np, dtype=torch.float32)
    except Exception:
        coords = None
...
# -----------------------------------------------------------------------------
# 3) Custom Dataset for multi‐label + optional fingerprint + descriptors
# -----------------------------------------------------------------------------
class OdourDatasetEnhanced(Dataset):
    def __init__(self, df: pd.DataFrame, cfg: DictConfig):
        """
        df must contain:
          - cfg.data.smiles_column  (one SMILES string per row)
          - each of cfg.data.label_columns  (one column per label)
        """
        super().__init__()
        ...
        # Pre‐coerce labels to float32 and compute pos_weights:
        label_arr = df[self.label_cols].apply(pd.to_numeric, errors="coerce").fillna(0.0).astype("float32")
        label_counts = (label_arr > 0).sum(axis=0).values  # (# positives per label)
    ...
    def __getitem__(self, idx: int):
        ...
        x = fea["x"]             # (num_atoms, 108)
        pos = fea["pos"]         # (num_atoms, 3) or None
        mask = fea["mask"]       # (num_atoms,)
        ...
        # Extra global descriptors (4 dims: logP, TPSA, MolWt, NumRotatableBonds)
        mol = Chem.MolFromSmiles(smiles)
        descs = torch.tensor([
...
# -----------------------------------------------------------------------------
# 4) Custom collate_fn to pad EGNN batches
# -----------------------------------------------------------------------------
def collate_egnn(batch: list):
    ...
    Output: one dict with batched tensors:
      - x:         (B, N_max, 108)
      - pos:       (B, N_max, 3)
      - mask:      (B, N_max)
      - edge_index, edge_attr not used by EGNN – we rely on fully‐connected or radius edges
      - y:         (B, num_labels)
      - descs:     (B, 4)
      - fp:        (B, fp_dim) or None
    """
    ...
    for i, item in enumerate(batch):
        n_i = item["x"].shape[0]
        x_batch[i, :n_i, :] = item["x"]           # (n_i, 108)
        mask_batch[i, :n_i] = item["mask"]        # (n_i,)
        if item["pos"] is not None:
            pos_batch[i, :n_i, :] = item["pos"]   # (n_i, 3)
        y_batch[i] = item["y"]                    # (num_labels,)
        descs_batch[i] = item["descs"]            # (4,)
        if use_fp:
            fp_batch[i] = item["fp"]              # (fp_dim,)
...
# -----------------------------------------------------------------------------
# 5) EGNN‐based Model + Fusion Heads
# -----------------------------------------------------------------------------
class OdourPredictorEGNN(nn.Module):
    def __init__(self, cfg: DictConfig, num_labels: int):
        ...
        # )
        num_layers = getattr(cfg.model, "egnn_layers", 3)  # add this key to config4 under model: egnn_layers: 3
    ...
    def forward(self, x, pos, mask, fp=None, descs=None):
        ...
        for layer in self.egnn_layers:
            fea, coor = layer(fea, coor, mask=mask)
        fea_out = fea  # final node features
...
def train_loop(cfg: DictConfig):
    ...
    # Build X = [SMILES + labels], then do a stratified multi-label split
    X_full = df[[cfg.data.smiles_column] + cfg.data.label_columns].copy()

    # ─── 6A) Multi-label stratified split
    ...
    # ─── 6B) Build PyTorch Dataset & DataLoader
    ds_train = OdourDatasetEnhanced(df_train, cfg)
    ds_val = OdourDatasetEnhanced(df_val, cfg)

    # Positional weights for weighted BCE
    pos_weights = ds_train.pos_weights.to(device)  # (num_labels,)
    ...
    # ─── 6C) Instantiate model, optimizer, scheduler, loss
    ...
    for epoch in range(cfg.train.epochs):
        ...
        for batch in train_loader:
            x = batch["x"].to(device)          # (B, N_max, 108)
            pos = batch["pos"].to(device)      # (B, N_max, 3)
            mask = batch["mask"].to(device)    # (B, N_max)
            y = batch["y"].to(device)          # (B, num_labels)
            descs = batch["descs"].to(device)  # (B, 4)
            fp = batch["fp"].to(device) if batch["fp"] is not None else None  # (B, fp_dim)
        ...
        all_logits = torch.cat(all_logits, dim=0).numpy()  # (N_val, num_labels)
        all_labels = torch.cat(all_labels, dim=0).numpy()  # (N_val, num_labels)

        # Compute per-label AUC and AP
        ...
        # Step scheduler on (1 - mean_auc) because we want to maximize AUC
