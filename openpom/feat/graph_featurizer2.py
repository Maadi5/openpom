...
from openpom.feat.base_classes_custom import MolecularFeaturizer
...
class GraphFeaturizer(MolecularFeaturizer):
    ...
    def _featurize(self, datapoint: RDKitMol, **kwargs) -> GraphData:
        ...
        f_atoms: np.ndarray = np.asarray(
            [atom_features(atom) for atom in datapoint['mol'].GetAtoms()],
            dtype=float)

        # get edge(bond) features
        if len(datapoint['mol'].GetBonds()) == 0:
            f_bonds: np.ndarray = np.empty((0, GraphConvConstants.BOND_FDIM))
        else:
            f_bonds_list = []
            for bond in datapoint['mol'].GetBonds():
                b_feat = 2 * [bond_features(bond)]
                f_bonds_list.extend(b_feat)
            f_bonds = np.asarray(f_bonds_list, dtype=float)

        # get edge index
        edge_index: np.ndarray = self._construct_bond_index(datapoint['mol'])

        return {'graph': GraphData(node_features=f_atoms,
                         edge_index=edge_index,
                         edge_features=f_bonds), 'fp_vec': datapoint['fp_vec']}
