...
molecule.
    ...
    def __init__(self, use_original_atoms_order=False, fp_dim = None, **kwargs):
        """
        Parameters
        ----------
    ...
    def smiles_to_fingerprint(self, smiles_val, n_bits):
        return AllChem.GetMorganFingerprintAsBitVect(Chem.MolFromSmiles(smiles_val), radius=2, nBits=n_bits)

    def featurize(self, datapoints, log_every_n=1000, fp_dim = None, **kwargs) -> np.ndarray:
        ...
        for i, mol in enumerate(datapoints):
            ...
            try:
                if isinstance(mol, str):
                    ...
                    if hasattr(self, 'use_original_atoms_order'
                        ...
                        if fp_dim is not None:
                            fp_vec = self.smiles_to_fingerprint(smiles_val= mol, n_bits = fp_dim)
                        else:
                            fp_vec = None
                ...
                # if fp_dim is not None:
                final_data = {'mol': mol, 'fp_vec': fp_vec}
                # else:
                #     final_data = mol
                features.append(self._featurize(final_data, **kwargs_per_datapoint))
