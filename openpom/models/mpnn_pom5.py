...
from openpom.models.torch_model4 import TorchModel
...
class MPNNPOM(nn.Module):
    ...
    def multi_graph(self, num):
        ...
        self.g = dgl.batch(list_of_copies).to('cpu')
    ...
    #
    #     node_encodings: torch.Tensor = self.mpnn(node_feats, edge_feats)
    #
    ...
    def forward(
            self, node_feats, edge_feats, multi
    ) -> torch.Tensor:
        """
...
class MPNNPOMModel(TorchModel):
    ...
    def _prepare_batch(
        ...
        _, labels, weights = super(MPNNPOMModel, self)._prepare_batch(
            ([], labels, weights))
        return g, node_feats, edge_feats, labels, weights
