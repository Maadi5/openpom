...
class TorchModel(Model):
    ...
    def fit_generator(self,
        ...
        for batch in generator:
            ...
            inputs, node_feats, edge_feats, labels, weights, fp_vecs = self._prepare_batch(batch)
            ...
            outputs = self.model(node_feats, edge_feats, fp_vecs, torch.tensor([1], device=self.device, dtype=torch.float32))
    ...
    def _predict(self, generator: Iterable[Tuple[Any, Any, Any]],
        ...
        for batch in generator:
            ...
            inputs, node_features, edge_features, _, _ = self._prepare_batch((inputs, None, None))

            # Invoke the model.
            if isinstance(inputs, list) and len(inputs) == 1:
                inputs = inputs[0]
            self.model.load_graph_obj(inputs)
            output_values = self.model(node_features, edge_features, torch.tensor([1], device= self.device))
