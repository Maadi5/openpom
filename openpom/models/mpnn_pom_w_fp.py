...
from openpom.models.torch_model5 import TorchModel
...
class EmbeddingLayer(nn.Module):
    def __init__(self, mpnn, readout, ffn, ffn_embeddings, ffn_output, readout_type='global_sum_pooling', fp_dim: int = 1024):
        ...
        self.fp_dim = fp_dim
        self.fp_out_dim = self.ffn_embeddings or 128
        ...
        combined_in_dim = self.ffn_embeddings + self.fp_out_dim
        self.combined_head = nn.Linear(combined_in_dim, self.ffn_embeddings)

    def forward(self, node_feats, edge_feats, fp_vector):
        ...
        fp_encoded = fp_encoded.squeeze(-1)
...
class MPNNPOM(nn.Module):
    def __init__(self,
        ...
        self.get_embeddings: nn.Module = EmbeddingLayer(mpnn=self.mpnn, readout=self._readout, ffn=self.ffn,
                                                   readout_type=self.readout_type, fp_dim=self.fp_dim, ffn_embeddings=self.ffn_embeddings, ffn_output= self.ffn_output)
    ...
    def _readout(self, node_encodings: torch.Tensor,
        ...
        self.g.send_and_recv(self.g.edges(),
    ...
    def forward(
            self, node_feats, edge_feats, fp_vec, multi
    ) -> torch.Tensor:
        """
        ...
        * When self.mode = 'classification',
        ...
        """
        self.multi_graph(multi)
        embeddings = self.get_embeddings(node_feats, edge_feats, fp_vec)
...
class MPNNPOMModel(TorchModel):
    ...
    def _prepare_batch(
        ...
        dgl_graphs: List[DGLGraph] = [
            graph['graph'].to_dgl_graph(self_loop=self._self_loop)
...
if __name__ == "__main__":
    ...
    input_file = r'/Users/adithyam.a/openpom/openpom/data/curated_datasets/curated_GS_LF_merged_4983.csv'  # or new downloaded file path
